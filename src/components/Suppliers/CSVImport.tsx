...
export default function CSVImport({ onClose }: CSVImportProps) {
  ...
  const downloadTemplate = () => {
    ...
    const sampleData = [
      ...
      'Art and History',
      'London',
      '123 Museum Street, London SW1A 1AA',
      '+44 20 7946 0958',
      'info@examplemuseum.com',
      'www.examplemuseum.com',
      'John Smith',
      '£15 per student',
      'Interactive exhibits, guided tours',
      'Group discounts available; educational workshops for schools',
      '30 minutes',
      'Underground',
    ...
    for (const row of data) {
      try {
        ...
        if (!row.name || !row.email || !row.phone) {
          result.errors.push(`Row with name "${row.name || 'Unknown'}" missing required fields (name, email, phone)`);
          continue;
        }
        ...
        const supplierData = {
          ...
          notes_for_groups: row.notes_for_groups || '',
    ...
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div className="relative top-20 mx-auto p-5 border w-11/12 max-w-2xl shadow-lg rounded-md bg-white">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900">Import Suppliers from CSV</h3>
          <button
            onClick={onClose}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h4 className="font-medium text-blue-900 mb-2">How to import suppliers:</h4>
              ...
              {result.success > 0 && (
                <div className="flex items-center p-3 bg-green-50 border border-green-200 rounded-md">
                  ...
                  <span className="text-green-800">
                    Successfully imported {result.success} suppliers
                  </span>
                </div>
              )}
              ...
              {result.duplicates > 0 && (
                <div className="flex items-center p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                  ...
                  <span className="text-yellow-800">
                    Skipped {result.duplicates} duplicate suppliers
                  </span>
                </div>
              )}
