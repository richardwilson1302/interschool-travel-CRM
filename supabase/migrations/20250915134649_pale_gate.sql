/*
  ...
  1. New Tables
    ...
    - `trips` - Tour offerings with pricing and dates
    - `bookings` - Booking records with status tracking
    - `suppliers` - Service providers (hotels, attractions, etc.)
    - `excursions` - Activities and attractions for trips
    - `booking_excursions` - Link between bookings and excursions
    - `activities` - Communication and interaction logs

  2. Custom Types
    - `booking_status` - Enum for booking pipeline stages
    - `activity_type` - Enum for different activity types
